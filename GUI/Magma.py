...
def imitovstavka(s):
	...
	return message_16code, otvet_imitovstavka
...
def prZamena(s):
	...
	return message16x, encode, decode16x, decode
...
def Gamma(s):
	...
	for j in range(0, len(enc_s) // 8):
		...
		for i in range(int(8 * j2), int(8 + (8 * j2))):
			...
			zzz += 1
		zzz = 0
	for kryaka in range(0, len(dec_s) - dop):
		dec_s_final += dec_s[kryaka]
	return s, enc_s, dec_s_final
...
def GammaOBR(s):
	...
	for j in range(0, len(s) // 8):
		...
		for i in range(8 * j2, 8 + 8 * j2):
			...
			zzz += 1
		zzz = 0
	for kryaka in range(0, len(decrypted_message) - dop):
		decrypted_message_final += decrypted_message[kryaka]
	return s, encrypted_message, decrypted_message_final
	# print("Расшифрованное сообщение:\n", decrypted_message)
