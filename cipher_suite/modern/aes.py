...
def AES_realize(way, key):
    ...
    if way == '1':
        ...
        input_path2 = os.path.abspath("crypted_file.txt")
...
def run_aes_cli():
    """Интерактивное шифрование/расшифрование файла (как в историческом AES.py)."""
    print("Шаг 1:")
    while True:
    ...
    while True:
        key = input(
            "Введите ваш ключ для зашифровки / расшифровки.\n"
    ...
    if way == "1":
        ...
        for byte in data:
            ...
            if 0 < len(temp) < 16:
                empty_spaces = 16 - len(temp)
                for _i in range(empty_spaces - 1):
                    temp.append(0)
                temp.append(1)
                crypted_part = encrypt(temp, key)
                crypted_data.extend(crypted_part)
        ...
        out_path = os.path.join(
            os.path.dirname(input_path), "crypted_" + os.path.basename(input_path)
        )
        ...
        out_path = os.path.join(
            os.path.dirname(input_path), "decrypted_" + os.path.basename(input_path)
        )
